...
window.addEventListener("mousemove", (e) => {
  if (!introAnimationIsPlaying) {
      onPointerMove(e.clientX, e.clientY)  // 改为clientX/Y
  }
});
window.addEventListener("touchmove", (e) => {
  if (!introAnimationIsPlaying) {
      const touch = e.targetTouches[0];
      onPointerMove(touch.clientX, touch.clientY)  // 改为clientX/Y
  }
});
...
function introAnimation() {
  ...
  gsap.timeline({
      ...
      .to(mouseCoords, {
          x: .4 * window.innerWidth,  // 使用视口宽度
          ease: "power1.out"
      })
      .to(mouseCoords, {
          y: .6 * window.innerHeight, // 使用视口高度
          ease: "back.out(3)"
      }, 0);
}
